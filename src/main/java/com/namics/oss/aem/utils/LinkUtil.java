...
public class LinkUtil {
     ...
     * the relative path is returned without any changes.
     *
     ...
     */
    public static boolean isMissingHtmlExtension(String path) {
        if (StringUtils.isEmpty(path) || isLinkToDAM(path)) {
            return false;
        }
        return path.startsWith(Links.SLASH) && !path.matches(".+\\.html(|\\?.*|#.*)$") && !path.matches(".+/(([#][^#/?]*)?|([?][^#/?]*))$");
    }
